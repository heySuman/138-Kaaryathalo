...
export default function FreelancerProfileDialog({ freelancer }: { freelancer: IFreelancer | null }) {
    ...
    const { data, setData, post, patch, processing, errors, reset } = useForm<{
        ...
        _method: freelancer ? 'PATCH' : 'POST',
    });

    const [isOpen, setIsOpen] = useState(false);
    const [previewUrl, setPreviewUrl] = useState<string | null>(freelancer?.profile_picture || null);
    ...
    const submit = (e: FormEvent) => {
        ...
        if (freelancer) {
            ...
            post(route('freelancer.profile.update'), {
                data: formData,
                forceFormData: true,
                onSuccess: () => {
            ...
            post(route('freelancer.profile.create'), {
                data: formData,
                forceFormData: true,
                onSuccess: () => {
    ...
    return (
        <AlertDialog open={isOpen} onOpenChange={setIsOpen}>
            <AlertDialogTrigger asChild>
                <Button variant={freelancer ? 'ghost' : 'default'} className="border" size="sm" onClick={() => setIsOpen(true)}>
                    {freelancer ? <Pencil /> : 'Create Profile'}
                </Button>
            </AlertDialogTrigger>
            ...
            {isOpen && (
                <AlertDialogContent className="max-h-[90vh] max-w-3xl overflow-y-auto">
                    <AlertDialogHeader>
                        <AlertDialogTitle>{freelancer ? 'Edit Your Freelancer Profile' : 'Create Your Freelancer Profile'}</AlertDialogTitle>
                        ...
                        <form onSubmit={submit} encType="multipart/form-data" className="flex flex-col gap-4">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <Label htmlFor="profile_picture" className="cursor-pointer hover:underline">
                                        {freelancer && previewUrl ? 'Change Profile Picture' : 'Upload Profile Picture'}
                                    </Label>
                                    <Input type="file" id="profile_picture" accept="image/*" onChange={handleFileChange} className="mt-1" />
                                    {errors.profile_picture && <p className="mt-1 text-red-500">{errors.profile_picture}</p>}
                                </div>
                            </div>
                            ...
                            <AlertDialogFooter>
                                ...
                                <Button type="submit" disabled={processing}>
                                    {processing ? 'Saving...' : freelancer ? 'Update Profile' : 'Create Profile'}
                                </Button>
                            </AlertDialogFooter>
                        </form>
                    </AlertDialogHeader>
